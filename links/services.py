    ...
    def fetch_report(cls, url, strategy='mobile'):
        ...
        try:
            ...
            if e.response.status_code == 429:
                raise Exception("Google API quota exceeded. Please try again later or check your API usage in the Google Cloud Console.")
            elif e.response.status_code == 400:
                raise Exception("Invalid URL or request parameters. Please check the URL and try again.")
            elif e.response.status_code == 403:
                raise Exception("Access forbidden: Your API key may be invalid, restricted, or over quota. Check your Google Cloud Console settings.")
            else:
                raise Exception(f"Google API HTTP error: {e.response.status_code} {e.response.reason}. Details: {e.response.text}")
    ...
    def fetch_and_store_report_group(cls, url, user):
        ...
        for psi_report, data in [(mobile_report, mobile_data), (desktop_report, desktop_data)]:
            ...
            FieldMetrics.objects.create(
                ...
                ttfb_ms=metrics.get('EXPERIMENTAL_TIME_TO_FIRST_BYTE', {}).get('percentile'),
